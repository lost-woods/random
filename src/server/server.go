...
func New(port string, r rng.Reader, h *rng.Health, log *zap.SugaredLogger) *Server {
    gin.SetMode(gin.ReleaseMode)
    router := gin.Default()

    // Background health monitoring (best-effort)
    // Interval is configurable via RNG_HEALTH_INTERVAL_MS (default 10000ms).
    interval := 10 * time.Second
    if msStr := os.Getenv("RNG_HEALTH_INTERVAL_MS"); msStr != "" {
    ...
    router.Use(cors.New(cors.Config{
        ...
        AllowHeaders:     []string{"X-API-KEY", "Accept", "Content-Type"},
